...
- ok This empty directory is where you should create a docker-compose.yml
- ok Use the `drupal:9` image along with the `postgres:14` image
- 2020 UPDATE: No version needed in compose files! All 2.x and 3.x features supported without version key.
- ok Use `ports` to expose Drupal on 8080 (or your preferred port for localhost access)
- ok Be sure to setup the POSTGRES_PASSWORD environment variable on the postgres service
